...
const SUPPORT: PolygonalCoverage = { type: "Polygon", coordinates: [[[-74.5, 40.3], [-73.4, 40.3], [-73.4, 41.1], [-74.5, 41.1], [-74.5, 40.3]]] };
...
export async function assembleReceipts(): Promise<{ receiptCount: number; rawAssetCount: number; receiptPath: string; acquisitionManifestPath: string }> {
  ...
  const workloads: Receipt[] = await Promise.all(Object.entries(WORKLOADS).map(async ([id, [filename, bbox]]) => ({
    ...
    assets: [await asset(raw, filename, "https://overpass-api.de/api/interpreter", "Overpass API snapshot recorded inside raw response", "Overpass JSON (out body geom)", "EPSG:4326", "AGL", rectangle(bbox))],
  })));
  const receipts: Receipt[] = [
    ...
    { id: "overture-buildings", role: "buildings", licence: "ODbL 1.0", rights: "https://docs.overturemaps.org/attribution/", assets: [await asset(raw, "overture-2026-08-19-buildings-nyc-support.parquet", `${OVERTURE_BASE}/type=building/`, `Overture ${OVERTURE_RELEASE}, selected with official overturemaps client`, "GeoParquet 1.1", "EPSG:4326", "AGL", SUPPORT)], normalization: { kind: "buildings", selection: `official overturemaps client; release=${OVERTURE_RELEASE}; bbox=-74.50,40.30,-73.40,41.10; type=building; STAC partition selection; stable record-id order at normalization`, priority: "overture-then-osm", missingHeight: "reject", raisedStructure: "retain-conflict" } },
    { id: "overture-building-parts", role: "building-parts", licence: "ODbL 1.0", rights: "https://docs.overturemaps.org/attribution/", assets: [await asset(raw, "overture-2026-08-19-building-parts-nyc-support.parquet", `${OVERTURE_BASE}/type=building_part/`, `Overture ${OVERTURE_RELEASE}, selected with official overturemaps client`, "GeoParquet 1.1", "EPSG:4326", "AGL", SUPPORT)], normalization: { kind: "building-parts", selection: `official overturemaps client; release=${OVERTURE_RELEASE}; bbox=-74.50,40.30,-73.40,41.10; type=building_part; STAC partition selection; stable record-id order at normalization`, priority: "overture-then-osm", missingHeight: "reject", raisedStructure: "retain-conflict" } },
    ...
    { id: "chmv2-validity-mask", role: "canopy-mask", licence: "CC BY 4.0", rights: "https://creativecommons.org/licenses/by/4.0/", assets: [await asset(raw, "chmv2-validity-mask-nyc-support.geojson", `${CHM_BASE}/tiles.geojson`, "CHMv2 ml3 authoritative tile index, selected 2026-09-13", "GeoJSON tile-availability mask", "EPSG:4326", "AGL", { type: "MultiPolygon", coordinates: [...tiles.values()].map((geometry) => geometry!.coordinates as number[][][]) })], normalization: chmPolicy("chmv2-validity-mask") },
    { id: "osm-tree-fallback", role: "canopy-fallback", licence: "ODbL 1.0", rights: "https://www.openstreetmap.org/copyright", assets: [await asset(raw, "osm-fallback/osm-vegetation-nyc-support-overpass.json", "https://overpass.private.coffee/api/interpreter", "Overpass OSM snapshot 2026-07-24T11:04:51Z", "Overpass JSON (out body geom)", "EPSG:4326", "AGL", SUPPORT)], normalization: { kind: "osm-tree-fallback", policy: "only-when-chmv2-unavailable" } },
  ...
  await writeJson(acquisitionManifestPath, { schemaVersion: 1, createdAt: new Date().toISOString(), boundary: { filename: "raw/nyc-borough-boundaries-26b.geojson", sha256: await fileHash(join(raw, "nyc-borough-boundaries-26b.geojson")), release: "NYC DCP 26b" }, support: { filename: "acquisition/nyc-five-borough-20km-support.geojson", sha256: await fileHash(supportPath), bufferMetres: 20000, selectionEnvelope: SUPPORT }, receipts });
