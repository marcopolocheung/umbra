...
/**
 * Walking seconds from one origin to every node the pedestrian graph can
 * reach — the one search every transit access candidate shares, instead of one
 * `dijkstra` per candidate stop.
 *
 * Distances are metres of mode-adjusted walk (`walk` prohibits no edge, so the
 * traversal is exactly `dijkstra`'s); seconds come from `speedMps`. The
 * intersection-crossing penalty is deliberately **absent**: transit board and
 * alight snaps stand exempt from the crossing penalty in the existing
 * pipeline, and a shared search must not price a stop differently than the
 * dedicated walk it replaces.
 *
 * Returns only the seconds a caller needs to rank candidates and seed a
 * transit search; reconstructing the winning walk's geometry stays `dijkstra`'s
 * job, run once for the chosen stop.
 */
export function walkSecondsFrom(
  graph: RoutingGraph,
  startId: number,
  speedMps: number
): Map<number, number> {
  const dist = new Map<number, number>([[startId, 0]]);
  const heap = new MinHeap<{ id: number; cost: number }>((a, b) => a.cost - b.cost);
  heap.push({ id: startId, cost: 0 });
  while (heap.size > 0) {
    const { id, cost } = heap.pop()!;
    if (cost > (dist.get(id) ?? Infinity)) continue;
    for (const edge of graph.adj.get(id) ?? []) {
      const newCost = cost + modeAdjustedDistanceM(edge, "walk");
  ...
  for (const [id, metres] of dist) seconds.set(id, metres / speedMps);
  return seconds;
}

/**
 * The reverse search: walking seconds from every reachable node **to** one
 * destination. Built on a reversed adjacency, so one search serves every
 * egress candidate — the same sharing, the other direction.
 */
export function walkSecondsTo(
  graph: RoutingGraph,
  endId: number,
  speedMps: number
): Map<number, number> {
  // Reverse every edge once, then run the same single-source search over the
  // reversed adjacency: seconds-to-destination for every reachable node.
  const reverseAdj = new Map<number, GraphEdge[]>();
  for (const [from, edges] of graph.adj) {
    for (const edge of edges) {
      let list = reverseAdj.get(edge.toId);
      if (!list) {
        list = [];
        reverseAdj.set(edge.toId, list);
      }
      list.push({ ...edge, toId: from });
    }
  }
  return walkSecondsFrom(
    { nodes: graph.nodes, adj: reverseAdj },
    endId,
    speedMps,
  );
}
