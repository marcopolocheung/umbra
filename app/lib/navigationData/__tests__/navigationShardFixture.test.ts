...
// Checkpoint 6 benchmark pairs — inside the verified support of their fixture
// profiles after the route bbox's ≥0.008° padding (duplicated in
// `e2e/bench/scenarios.ts`, where the bench reads them; keep the two in sync).
const LONG_WAYPOINT_A: [number, number] = [-73.987002, 40.748925];
const LONG_WAYPOINT_B: [number, number] = [-73.944346, 40.788168];
const CROSS_WAYPOINT_A: [number, number] = [-73.984336, 40.750143];
const CROSS_WAYPOINT_B: [number, number] = [-73.94168, 40.843454];

function fixtureBoroughs() {
  const artifacts = buildNavigationShardFixture({ profile: "boroughs" });
  const pointer = parseNavigationPointer(JSON.parse(artifacts.pointer));
  const manifest = parseNavigationManifest(JSON.parse(artifacts.manifest), pointer.generation);
  return { artifacts, pointer, manifest };
}
...
describe("buildNavigationShardFixture — Checkpoint 6 boroughs profile", () => {
  it("pins the 8-cell city slice, distinct from the A4 generation", () => {
    const scale = buildNavigationShardFixture({});
    const { artifacts, pointer, manifest } = fixtureBoroughs();
    expect(manifest.generation).toBe(pointer.generation);
    expect(pointer.generation).not.toBe(JSON.parse(scale.pointer).generation);
    expect(manifest.streetShards).toHaveLength(BOROUGHS_COUNTS.streetShards);
    expect(pointer.manifestSha256).toBe(sha256(artifacts.manifest));
    expect(manifest.buildingShards).toHaveLength(1);
    expect(manifest.buildingShards[0].buildings).toBe(BOROUGHS_COUNTS.buildings);
  });

  it("keeps every boroughs street shard inside the contract's byte budget", () => {
    const { artifacts, manifest, pointer } = fixtureBoroughs();
    for (const ref of manifest.streetShards) {
      expect(ref.bytes).toBeLessThanOrEqual(MAX_STREET_SHARD_BYTES);
      const raw = artifacts.streetShards.get(ref.key);
      expect(raw).toBeDefined();
      // A shard must also stay under the manifest's own published budget once
      // every seam ghost is in (parsing validates the rest of the contract).
      const parsed = parseNavigationStreetShard(JSON.parse(raw!), ref, pointer.generation);
      expect(parsed.nodes.length).toBeGreaterThan(0);
      expect(parsed.edges.length).toBeGreaterThan(0);
    }
  });

  it("selects all 8 cells for the cross-borough pair and routes it", () => {
    const { artifacts, manifest, pointer } = fixtureBoroughs();
    const picked = selection(manifest, benchBbox(CROSS_WAYPOINT_A, CROSS_WAYPOINT_B));
    expect(picked.streets.length).toBe(BOROUGHS_COUNTS.streetShards);
    const graph = graphFor(artifacts, picked, pointer.generation);
    expect(graph.nodes.size).toBe(BOROUGHS_COUNTS.streetNodes);
    ...
    const from = nearest(CROSS_WAYPOINT_A);
    const to = nearest(CROSS_WAYPOINT_B);
    expect(from.distanceM).toBeLessThan(32);
    expect(to.distanceM).toBeLessThan(32);
    const route = dijkstra(graph, from.node.id, to.node.id, 0.5);
    expect(route, "cross-borough pair must route over the seeded city slice").not.toBeNull();
    expect(route!.distanceM).toBeGreaterThan(9000);
  });

  it("selects all 4 scale cells for the longer-Manhattan pair and routes it", () => {
    const { artifacts, manifest, pointer } = fixture();
    const picked = selection(manifest, benchBbox(LONG_WAYPOINT_A, LONG_WAYPOINT_B));
    expect(picked.streets.length).toBe(NAV_STATIC_COUNTS.streetShards);
    const graph = graphFor(artifacts, picked, pointer.generation);
    expect(graph.nodes.size).toBe(NAV_STATIC_COUNTS.streetNodes);
  });
