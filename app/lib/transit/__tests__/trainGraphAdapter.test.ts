...
// ─── Duplicate edges across shards ───────────────────────────────────────────

describe("duplicate edges published by overlapping borough shards", () => {
  /**
   * Both shards carry the far end of their boundary edge: the A→B hop appears
   * in each with identical bytes. The adjacency must hold it once, or every
   * duplicated hop times double and the search finds "alternatives" that are
   * the same service.
   */
  function overlappingShards(): TransitShard[] {
    const base = shard();
    const twin = { ...shard(), kind: "subway" as const };
    // Re-publish only the boundary hop in the twin, exactly as a neighbouring
    // borough shard would.
    twin.edges = [
      { from: "subway:A", to: "subway:B", route: "1", direction: 0, medianSec: 90, trips: 100, distM: 500 },
      { from: "subway:B", to: "subway:A", route: "1", direction: 1, medianSec: 90, trips: 100, distM: 500 },
    ];
    return [base, twin];
  }

  it("holds the duplicated hop once in the adjacency", () => {
    const graph = buildTrainGraphFromShards(overlappingShards())!;
    const aToB = graph.adj.get("subway:A")!.filter(
      (edge) => edge.to === "subway:B" && edge.line === "1" && edge.direction === 0,
    );
    expect(aToB).toHaveLength(1);
  });

  it("prices the duplicated ride once, not per publishing shard", () => {
    const graph = buildTrainGraphFromShards(overlappingShards())!;
    const path = trainDijkstra(graph, "subway:A", "subway:C")!;
    expect(path.stationIds).toEqual(["subway:A", "subway:B", "subway:C"]);
