...
case "$file" in
  *.tsx|*.css)
    if [[ -f "$root/scripts/verify/design-tokens.mjs" ]]; then
      ...
      # The inventory-clearing flag: registry violations are advisory until Track U2 lands
      # docs/design/language.md. The hook keeps reporting; the wave's gate turns it hard.
    fi
    ;;
esac
