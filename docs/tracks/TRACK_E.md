...
- **Active checkpoint:** E5 — the `Trip` model — **PR #372 open**. G6a merged (#363), which was its precondition
...
- **Next action:** get the browser check done on #372 (see Last verified), then hand E5 off to C11 — the `Trip` is its plan carrier, and `useNavigation` now returns it; then E7 slices (a)–(c) against a fixture stub
