...
- **Active checkpoint:** U0 — land the seeded harness.
- **Implemented:** nothing. The seeded harness files (§Harness) live on branch
  `design/u0-harness` with an open, unreviewed PR; U0 is the checkpoint that lands them.
- **Owner decisions** D1–D12, 2026-09-19, in `docs/handoffs/DESIGN_LANGUAGE.md` §1.
- **Last verified:** 2026-09-19 at `0a28a09` — token layer `--md-*` in `app/globals.css`;
  `SearchBar.tsx` 16 token refs vs `RouteCard.tsx` 35; literals in
  `NavigationPanel.tsx:233,799`, `DirectionsPanel.tsx:21,31,504,558`, `app/about/page.tsx:5`.
  **Merged design work supersedes this** — the language doc (`docs/design/language.md`) becomes
  the source of truth once U2 lands.
