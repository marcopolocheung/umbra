...
wait — and reads `N% shadow on foot`, or `shadow unknown` if any stop went unanswered. The ride
stays out of the headline and the dose, and the card says so in words; so does an unmodelled
platform wait. Other panels that print `shadowCoverage` do not know the unknown state yet (#434).*
